...
export class MRMService {
  ...
  async addDocumentToOS(files, user, randomName, id) {
    ...
    this.logger.debug(`adding document to os service started (MRM service)`);
    if (process.env.IS_OBJECT_STORE === 'true') {
      ////console.log('inside');
      this.logger.debug(`inside OS part`);
      const fs = require('fs');
      this.logger.debug(
        `fetching obect storage credentails from connected apps config`,
      );
    ...
    // this part should be called when object storage is set to false
      ...
      this.logger.debug(`inside local storage part`);
      const uploadedFiles = await this.uploadAttachment(files, activeUser, id);
      return uploadedFiles;
    }
  }
  ...
  async getLocationforOrg(realmName: string, user: any) {
    ...
    try {
      this.logger.debug(`getLocationforOrg service started for ${realmName}`);
      const organization = await this.prisma.organization.findFirst({
        where: {
          realmName: realmName,
        },
      });
      this.logger.debug(`fetched organization ${organization}`);
      const activeUser = await this.prisma.user.findFirst({
        where: {
          kcId: user?.id,
        },
      });
      this.logger.debug(`fetched activeser ${activeUser}`);
      if (activeUser?.userType !== 'globalRoles') {
        this.logger.debug(`inside usetype is not global user`);
        if (user.kcRoles?.roles?.includes('ORG-ADMIN')) {
          if (organization) {
            ...
            this.logger.debug(
              `return all locations if user has org admin roles ${location?.length}`,
            );
            return location;
          } else {
          ...
          this.logger.debug(`user has mr role`);
          const otherUser = await this.userService.getUserInfo(user.id);
          let locId: any = otherUser.locationId ?? otherUser.entity.locationId;
          locId = [...new Set([locId, ...otherUser.additionalUnits])];
          const locationOfTheUser = await this.prisma.location?.findMany({
            where: {
              id: { in: locId },
              deleted: false,
            },
          ...
          locationOfTheUser.sort((a: any, b: any) =>
            a.locationName
              .toLowerCase()
              .localeCompare(b.locationName.toLowerCase()),
          );
          this.logger.debug(
            `returnnig mr locations ${locationOfTheUser?.length}`,
          );
          return locationOfTheUser;
        } else {
          this.logger.debug(`inside normal user part`);
          const otherUser = await this.userService.getUserInfo(user.id);
          const locId = otherUser.locationId ?? otherUser.entity?.locationId;
          const locationOfTheUser = await this.prisma.location?.findFirst({
            where: {
              id: locId,
              deleted: false,
            },
          ...
          if (!locationOfTheUser) {
            throw new NotFoundException();
          }
          this.logger.debug(`location for normal user ${locationOfTheUser}`);
          return [locationOfTheUser];
        }
      } else {
        let locationOfTheUser;
        if (activeUser?.additionalUnits?.includes?.('All')) {
          locationOfTheUser = await this.prisma.location?.findMany({
            where: {
              organizationId: activeUser?.organizationId,
              deleted: false,
            },
        ...
        this.logger.debug(`global user lcoations ${locationOfTheUser}`);
        return locationOfTheUser;
      }
    } catch (error) {}
  }
  ...
  async sendMailToParticipants(
    ...
    this.logger.debug(
      `sendMail tio particpants mailing started for emails= ${attendeesEmails} scheudle=${meetingSchedule}owner= ${owner}`,
    );
