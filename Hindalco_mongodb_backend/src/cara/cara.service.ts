...
export class CaraService {
  ...
  async getAllCara(query, user, randomNumber) {
    try {
      ...
      if (query.entityId !== 'All') {
        filterCriteria.entityId = query.entityId;
      }
      ...
      this.logger.debug(`Total matching CARAs: ${count}`, 'Cara-controller');

      let data = [];
      let owner;

      for (let cara of result) {
        this.logger.debug(`Processing CARA ID: ${cara._id}`, 'Cara-controller');

        const actionItem = await this.actionItemModel.find({
          organizationId: activeuser.organizationId,
          source: 'CAPA',
        ...
        const cordinator: any = await this.prisma.user.findFirst({
          where: { id: cara.caraCoordinator },
          select: {
            id: true,
            firstname: true,
        ...
        this.logger.debug(
          `CARA data composed: ${JSON.stringify(data1)}`,
          'Cara-controller',
        );
  ...
  async searchCapa(query, user, randomNumber) {
    try {
      ...
      this.logger.debug(
        `Final Query Conditions: ${JSON.stringify(queryConditions)}`,
        'Cara-service',
      );

      const newRes: any = await this.caraModel
        .find({ $and: queryConditions })
        .skip(skip)
        .limit(limit)
        .lean();

      const count = await this.caraModel.countDocuments({
        $and: queryConditions,
      });

      this.logger.debug(
        `CAPAs Fetched: ${newRes.length}, Count: ${count}`,
        'Cara-service',
      );
  ...
  async getEntitiesForLocation(id, uuid, user) {
    try {
      ...
      this.logger.debug(
        `trace id = ${uuid} Active user fetched: ${JSON.stringify(activeuser)}`,
        'Cara-service',
      );
      ...
      this.logger.debug(
        `trace id = ${uuid} EntityTypeId for 'Department': ${JSON.stringify(
          entityTypeId,
        )}`,
        'Cara-service',
      );
  ...
  async myCapa(query, userid, uuid) {
    try {
      ...
      if (query.selectedOrigin?.length) {
        filterCriteria.origin = { $in: query.selectedOrigin };
      }
      if (query.selectedOwner?.length) {
        filterCriteria.caraOwner = { $in: query.selectedOwner };
      }
      if (query.selectedStatus?.length) {
        filterCriteria.status = { $in: query.selectedStatus };
      }
      ...
      this.logger.debug(
        `trace id = ${uuid} Final filterCriteria: ${JSON.stringify(
          filterCriteria,
        )}`,
        'Cara-service',
      );
      ...
      this.logger.debug(
        `trace id = ${uuid} Found ${caras.length} CAPA entries`,
        'Cara-service',
      );
      ...
      for (let cara of caras) {
        this.logger.debug(
          `trace id = ${uuid} Processing CAPA ID: ${cara._id}`,
          'Cara-service',
        );
  ...
  async getStatusOptionsForCapa(id, userid, uuid) {
    try {
      ...
      this.logger.debug(
        `trace id = ${uuid} CAPA details: ${JSON.stringify(capaDetails)}`,
        'capa.service.ts',
      );
      ...
      this.logger.debug(
        `trace id = ${uuid} Final status options: ${JSON.stringify(
          optionsArray,
        )}`,
        'capa.service.ts',
      );
  ...
  async getDeptwiseChartData(user, randomNumber, query) {
    try {
      ...
      if (!activeuser?.organizationId) {
        this.logger.warn(
          `trace id = ${randomNumber} → User has no associated organization`,
          'capa.service.ts',
        );
        return { deptwiseData: [] };
      }
