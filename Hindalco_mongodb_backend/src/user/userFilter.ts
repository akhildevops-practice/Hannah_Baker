...
export const findAllUsers = async (
  ...
  const filterQuery = filterString ? queryGeneartorForDocumentsFilter(applyFilters(filterString)) : [];
  ...
  const finalQuery = {
    ...baseQuery,
    ...searchQuery,
    ...(location?.length ? { locationId: { in: location } } : {}),
    ...(entity?.length ? { entityId: { in: entity } } : {}),
    ...(locationQuery ? { locationId: { contains: locationQuery?.id, mode: 'insensitive' } } : {}),
    ...(entityQuery ? { entityId: { contains: entityQuery?.id, mode: 'insensitive' } } : {}),
