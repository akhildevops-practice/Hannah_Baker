...
export class UserService {
  ...
  async createUser(payload: UserMaster, token: string, res: any) {
    ...
    if (createUserResponse.status === 201) {
      ...
      if (userType === 'globalRoles') {
        ...
        // console.log('created global roles payload', globalRolesData);
        const rolesResult = await this.globalRolesModel.create(globalRolesData);
        //console.log('globalresultdata', rolesResult);
      }
      const sendInviteKcApi =
        process.env.KEYCLOAK_API +
  ...
  async uploadAvatar(file: any, id: string) {
    ...
    if (process.env.IS_OBJECT_STORE === 'true') {
      path = await this.addDocumentToOS(file, locationName);
    } else {
      path = locationName
        ? `${realmName.organizationName.toLowerCase()}/${locationName.toLowerCase()}/avatar/${
            file.filename
          }`
        : `${realmName.organizationName.toLowerCase()}/avatar/${file.filename}`;
    }
  ...
  async getUserByRoleInfoById(id, user, query) {
    try {
      ...
      const aspectImpactData = await db
        .collection('aspimpconsolidatedstatuses')
        .find({
          $or: [{ reviewers: id }, { approvers: id }],
          deleted: false,
        })
        ?.toArray();
      // const entityIds: any = aspectImpactData.map((item: any) => {
      //   item.entityId;
      // });
      ...
      aspectImpactData?.map(async (item) => {
        const roleData = item?.reviewers?.includes(id)
          ? ['REVIEWER']
          : item?.reviewers?.includes(id)
          ? ['APPROVER']
          : '';
        // console.log('roledA', roleData);
        ...
        // });
        // console.log('entityNsme', entityName);
        result.push({
          type: 'Aspect Impact',
          // name: entityMap.get(item.entityId),
          number: item?.prefixSuffix,
          roles:
            item.reviewers?.includes(id) && item?.approvers?.includes(id)
              ? ['REVIEWER', 'APPROVER']
  ...
  async updateRoleforOtherUser(datas, user) {
    try {
      ...
      for (let i of data) {
        ...
        } else if (i.type === 'Aspect Impact') {
          if (i.roles?.includes('REVIEWER')) {
            const doc = await db
              .collection('aspimpconsolidatedstatuses')
              .updateOne(
                {
                  _id: new ObjectId(i.id), // Assuming `id` is the unique identifier of the document
                  reviewers: { $in: [id.id] }, // Ensure the old userId exists in the reviewers array
                },
                [
                  {
                    $set: {
                      reviewers: {
                        $cond: {
                          if: { $in: [id.id, '$reviewers'] }, // Check if the old id is in the reviewers array
                          then: {
                            $concatArrays: [
                              {
                                $filter: {
                                  input: '$reviewers',
                                  as: 'reviewer',
                                  cond: { $ne: ['$$reviewer', id.id] },
                                },
                              },
                              [userData?.id], // Add new userId
                            ],
                          },
                          else: '$reviewers', // If id doesn't exist, leave the reviewers array unchanged
                        },
                      },
                    },
                  },
            ...
            const doc = await db
              ...
              .updateOne(
                {
                  _id: new ObjectId(i.id), // Assuming `id` is the unique identifier of the document
                  approvers: { $in: [id.id] }, // Ensure the old userId exists in the reviewers array
                },
                [
                  {
                    $set: {
                      approvers: {
                        $cond: {
                          if: { $in: [id.id, '$approvers'] }, // Check if the old id is in the reviewers array
                          then: {
                            $concatArrays: [
                              {
                                $filter: {
                                  input: '$approvers',
                                  as: 'approver',
                                  cond: { $ne: ['$$approver', id.id] },
                                },
                              },
                              [userData?.id], // Add new userId
                            ],
                          },
                          else: '$approvers', // If id doesn't exist, leave the approvers array unchanged
                        },
                      },
                    },
                  },
