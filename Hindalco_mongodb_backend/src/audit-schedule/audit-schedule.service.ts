...
export class AuditScheduleService {
  ...
  async createAuditSchedule(data, userId) {
    ...
    try {
      ...
      try {
        const mail = await this.sendMailForHead(userId, result._id);
      } catch (error) {
  ...
  async sendMailForHead(user, id) {
    ...
    // if (getallentitydata.length > 0 && auditSchedule?.isDraft === false) {
    ...
    //     const entityData = await this.prisma.entity.findFirst({where:{
    //       id:rowdata?.entityId
    //     }})
    ...
    if (getallentitydata.length > 0 && auditSchedule?.isDraft === false) {
      ...
      for (const rowdata of getallentitydata) {
        // Collect unique user IDs for this row
        const entityData = await this.prisma.entity.findFirst({
          where: { id: rowdata?.entityId },
        });
        const additionalAuditee = entityData?.notification?.map(
          (ele: any) => ele?.id,
        );
        const userIds = [
          ...new Set([
        ...
        // Fetch all users in parallel, and cache them
        await Promise.all(
        ...
        // Resolve auditor and auditee user details from cache
        const auditor = rowdata.auditor
          .map((id) => userCache.get(id))
          .filter(Boolean);
        const auditee = rowdata.auditee
          .map((id) => userCache.get(id))
          .filter(Boolean);
        ...
        // Unique emails from auditor + auditee
        const uniqueRecipients: any = [
          ...new Map(
            [...auditor, ...auditee].map((u) => [u.email, u]),
          ).values(),
        ];
  ...
  async dropAndAuditPlan(user, data) {
    ...
    const additionalAuditee = entityData?.notification?.map((ele) => ele?.id);
    ...
    const [auditors, auditees, additionalAuditeeData] = await Promise.all([
      fetchUserDetails(auditScheduleEntityData.auditor),
      fetchUserDetails(auditScheduleEntityData.auditee),
      fetchUserDetails(additionalAuditee || []),
    ]);
  ...
  async sendMailtoDateChange(data: any, user) {
    ...
    const userIds = [
      ...
      ...data.auditor,
      user.id,
      entityData?.notification?.map((ele: any) => ele?.id),
    ];
