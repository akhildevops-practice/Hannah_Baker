    ...
    try {
      ...
      for (const location of allLocation) {
        ...
        if (previousData) {
          ...
          aspImpEstimateTotal = getLocationReport?.aspImp?.estimatedTotal;
        }
        // Get Document Count
        ...
        const totalAspImp = await this.aspImpModel.countDocuments(riskQuery);
        ...
        const report = {
          ...
          aspImp: {
            estimatedTotal: aspImpEstimateTotal,
            currentTotal: totalAspImp,
          },
          objAndKpi: {
    ...
    try {
      ...
      const updatedDocumentsData = await Promise.all(
        uniqueLocationResults.map(async (document: any) => {
          ...
          // Get Total Upload for Aspect Impact
          const totalUploadedAspImp =
            document.aspImp.estimatedTotal > 0
              ? (
                  (document.aspImp.currentTotal /
                    document.aspImp.estimatedTotal) *
                  100
                ).toFixed(2)
              : 0;
          ...
          return {
            ...
            aspImp: {
              ...document.aspImp,
              totalUploaded: totalUploadedAspImp,
              score: await this.getScore(totalUploadedAspImp),
            },
            objAndKpi: {
              ...document.objAndKpi,
