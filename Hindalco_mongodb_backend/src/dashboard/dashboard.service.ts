...
export class DashboardService {
  ...
  async findAllBydocuments(filterObj: DashboardFilter, user) {
    ...
    for (let document of filteredDocuments) {
      ...
      transformedData.push({
        ...
        sectionName: section === null ? null : section.name,
        isCreator: isUserCreator,
        readAccess,
      });
    }
  ...
  async findWorkFlowdocuments(filterObj: DashboardFilter, user) {
    const traceId = uuid();
    this.logger.log(
      `trace id=${traceId} Get api/dashboard/workFlowDocuments service started`,
      '',
    );
    ...
    const skip = (filterObj.page - 1) * Number(filterObj.limit);
    ...
    const where: any = {
      AND: [
        ...
        {
          documentState: {
            in: ['DRAFT', 'IN_REVIEW', 'IN_APPROVAL', 'SEND_FOR_EDIT'],
          },
        },
      ],
    };
    ...
    if (
      ...
      where.locationId = activeUser?.locationId;
      where.entityId = activeUser?.entityId;
    } else if (!activeUser?.additionalUnits?.includes?.('All')) {
      where.locationId = { in: activeUser?.additionalUnits || [] };
    }

    // Dynamic filters
    if (filterObj.dept?.length) {
      where.entityId = { in: filterObj.dept };
    }

    if (filterObj.documentTypes?.length) {
      where.docType = { in: filterObj.documentTypes };
    }

    if (filterObj.system?.length) {
      where.system = { hasSome: filterObj.system };
    }

    if (filterObj.section?.length) {
      where.section = { in: filterObj.section };
    }

    if (filterObj.documentStatus?.length) {
      where.documentState = { in: filterObj.documentStatus };
    }

    if (filterObj.user?.length) {
      where.OR = [
        { creators: { hasSome: filterObj.user } },
        { reviewers: { hasSome: filterObj.user } },
        { approvers: { hasSome: filterObj.user } },
      ];
    }

    if (filterObj.searchQuery && filterObj.searchQuery !== 'undefined') {
      where.OR = [
        {
          readAccess: { contains: filterObj.searchQuery, mode: 'insensitive' },
        },
        { docType: { contains: filterObj.searchQuery, mode: 'insensitive' } },
        {
          currentVersion: {
            contains: filterObj.searchQuery,
            mode: 'insensitive',
    ...
    const stateToTypeMap = {
      DRAFT: 'CREATOR',
      IN_REVIEW: 'REVIEWER',
      IN_APPROVAL: 'APPROVER',
      SEND_FOR_EDIT: 'CREATOR',
    };

    const filteredDocs = documents.filter((doc) => {
      const relevantType = stateToTypeMap[doc.documentState];
      const pendingWith = doc.AdditionalDocumentAdmins.map((ele) => {
        return { ...ele, userName: `${ele?.firstname} ${ele?.lastname}` };
      }).filter((a) => a.type === relevantType);
      const pendingWithCheck = pendingWith
        ?.map((item: any) => item?.userId)
        .some((val: any) => filterObj?.user?.includes(val));

      doc['pendingWith'] = pendingWith;

      return !filterObj.user?.length || pendingWithCheck;
    });
    ...
    const documentsWithPermissions = await Promise.all(
      filteredDocs.map(async (doc: any) => {
        const [
          ...
          this.SystemModel.find({ _id: doc?.system }).select('name _id type'),
          this.documentsService.checkPermissionsForPreviewPage(user, doc?.id),
          this.documentsService.checkIfUserCreatorForDocument(user, doc?.id),
          this.accesforDocument(doc.id, user),
          doc?.section
            ? this.prisma.section.findFirst({
                where: { organizationId: orgId, id: doc.section },
              })
            : null,
        ]);
        ...
        return {
          id: doc.id,
          documentNumbering: doc?.documentNumbering,
          documentName: doc?.documentName,
          documentType: doc?.doctype?.documentTypeName || '',
          issueNumber: doc?.issueNumber,
          version: doc?.currentVersion,
          status: doc?.documentState,
          system: systemData,
          department: doc?.creatorEntity?.entityName,
          location: doc?.creatorLocation?.locationName,
          distributionList: doc?.distributionList,
          distributionUsers: doc?.distributionUsers,
          approvedDate: doc?.approvedDate,
          isVersion: doc?.isVersion,
          pendingWith: doc?.pendingWith,
          section: doc?.section,
          sectionName: section?.name || null,
          access: user.kcRoles?.roles?.includes('ORG-ADMIN')
            ? true
            : access?.access,
          isCreator: user.kcRoles?.roles?.includes('ORG-ADMIN')
            ? true
            : isUserCreator,
          editAcess,
          deleteAccess,
    ...
    const totalCount = await this.prisma.documents.count({ where });

    this.logger.log(
      `trace id=${traceId} Get api/dashboard/workFlowDocuments service successful`,
      '',
    );
 ...
 async documentFilterList(user, filter, data) {
  const traceId = uuid();
  // this.logger.log(`trace id=${traceId} documentFilterList service started`);
  ...
  try {
    ...
    let whereCondition: any = { organizationId };
    const pendingWithMap = new Map();

    // === Build WHERE Condition Based on Filter ===
    switch (filter) {
      case 'myDoc': {
        const docIds = await this.prisma.additionalDocumentAdmins.findMany({
          where: { userId: activeUser.id },
          select: { documentId: true },
        }).then(res => res.map(r => r.documentId));
        whereCondition.id = { in: docIds };
        break;
      }

      case 'myFavDocs': {
        const favs = await this.favoriteService.getFavoriteByUserId(activeUser.id);
        const docIds = favs.map(d => d.id);
        whereCondition.id = { in: docIds };
        break;
      }
      ...
      case 'inWorkflow': {
        whereCondition.documentState = {
          in: ['DRAFT', 'IN_REVIEW', 'IN_APPROVAL', 'SEND_FOR_EDIT'],
        };

        const isOrgAdmin = user.kcRoles?.roles?.some(r =>
          ['ORG-ADMIN', 'MR'].includes(r),
        );

        if (isOrgAdmin) {
          whereCondition.locationId = activeUser.locationId;
        } else if (activeUser.userType === 'globalRoles') {
          ...
          if (activeUser.entityId) {
            whereCondition.entityId = activeUser.entityId;
          }
        }
        break;
      }
      ...
      case 'allDoc': {
        if (activeUser.userType === 'globalRoles') {
          if (!activeUser.additionalUnits?.includes('All')) {
            whereCondition.locationId = { in: activeUser.additionalUnits };
          }
    ...
    // === Collect Unique IDs and Values ===
    const docTypeIds:any = new Set();
    const statuses:any = new Set();
    const sectionIds:any= new Set();
    const systemIds:any = new Set();
    ...
    for (const doc of documents) {
      if (doc.doctypeId) docTypeIds.add(doc.doctypeId);
      if (doc.documentState) statuses.add(doc.documentState);
      if (doc.section && doc.section !== 'null') sectionIds.add(doc.section);
      if (Array.isArray(doc.system)) {
        doc.system.forEach(s => systemIds.add(s));
      }

      const entity = doc.creatorEntity;
      if (entity?.id && !entityMap.has(entity.id)) {
        entityMap.set(entity.id, { id: entity.id, name: entity.entityName });
      }

      const location = doc.creatorLocation;
      if (location?.id && !locationMap.has(location.id)) {
        locationMap.set(location.id, { id: location.id, name: location.locationName });
      }

      if (filter === 'inWorkflow') {
        const expectedType = stateRoleMap[doc.documentState];
        doc.AdditionalDocumentAdmins?.forEach(admin => {
          if (admin.type === expectedType && !pendingWithMap.has(admin.userId)) {
            pendingWithMap.set(admin.userId, admin);
          }
        });
      }
    }
    ...
    const [doctypeData, sectionData, systemData]:any = await Promise.all([
      ...
      this.SystemModel.find({ _id: { $in: [...systemIds] } })
        .select({ id: 1, name: 1 })
        .sort({ name: 1 }),
    ]);
    ...
    const result = {
      doctype: doctypeData.map(d => d.documentTypeName),
      system: systemData.map(s => ({ id: s.id, name: s.name })),
      status: [...statuses],
      entity: Array.from(entityMap.values()).sort((a, b) =>
        a.name.localeCompare(b.name),
      ),
      location: Array.from(locationMap.values()).sort((a, b) =>
        a.name.localeCompare(b.name),
      ),
      pendingWith: Array.from(pendingWithMap.values()),
      section: sectionData,
    };

    // this.logger.log(`trace id=${traceId} documentFilterList service successful`);
    return result;
  } catch (err) {
    this.logger.error(`trace id=${traceId} documentFilterList failed: ${err.message}`);
    throw new InternalServerErrorException(err);
  }
}
