...
export class AuditService {
  ...
  async create(createAuditDto, kcId: any, req: any) {
    // try {
    const activeUser = await this.userService.getUserInfo(req.user.id);
          ...
          for (let j = 0; j < section.fieldset.length; j++) {
            ...
            if (field.nc.type !== '') {
              ...
              if (
                ...
                auditFindings?.closureBy === 'None' &&
                auditFindings?.autoAccept === false &&
                auditFindings?.correctiveAction === false &&
                auditFindings?.auditorVerification === false &&
                auditFindings?.reject === false
              ) {
                status = 'NA';
              } else if (
              ...
              nc = await this.createNewNC({
                ...
                auditedEntity: createAuditDto?.auditedEntity,
  ...
  async findOne(id: any, user) {
    ...
    const auditedDocuments = await Promise.all(
      audit.auditedDocuments.map((item) =>
        this.documentService.findOne(item, '', null, user),
      ),
    );
  ...
  async update(id: string, updateAuditDto: any, userDtls) {
    const user = await this.userService.getUserInfo(userDtls.user.id);
    const auditTrail = await auditTrial(
      'audits',
      'Audit',
      'Audit Report',
        ...
        for (let i = 0; i < checklist.sections.length; i++) {
          ...
          for (let j = 0; j < section.fieldset.length; j++) {
            ...
            if (field?.nc?.hasOwnProperty('_id')) {
              ...
              console.log('test2', field?.nc?.type);
              ...
              if (
                ...
                console.log('auditFindings', auditFindings?.findingType);
                let status = 'OPEN';
                ...
                const res = await this.createNewNC({
                  ...
                  auditedEntity: updateAuditDto.auditedEntity,
                  organization: user.organizationId,
  ...
  async search(query: any, kcId: string, year) {
    ...
    const filter = filterGenerator({
      ...
      selectedStatus:query?.selectedStatus?.length >0?query?.selectedStatus?.includes('Published')?false:true:undefined,
      myDept: query?.myDept,
      ...
      selectedAuditee:
        ...
        query.selectedAuditee !== 'undefined'
          ...
          : 'undefined',
      selectedDepartment:
       ( query.selectedDepartment !== undefined ||
        query.selectedDepartment !== '' ||
        query.selectedDepartment !== 'undefined')&& query.myDept==="false"
          ? query.selectedDepartment
          : 'undefined',
    });
    // counting documents
    ...
    try {
      ...
      if (process.env.IS_OBJECT_STORE === 'true') {
        ...
        const getObjectStoreContents =
          await this.prismaService.connectedApps.findFirst({
            where: {
  ...
  async getAttachmentUrlOBJ(documentLink) {
    try {
      const getObjectStoreContents =
        await this.prismaService.connectedApps.findFirst({
          where: {
  ...
  async accessRigts(audit, nc, user) {
    ...
    let status = false;
    if (nc.currentlyUnder === 'AUDITEE' && nc.status !== 'NA') {
  ...
  async sendPDFMailAttachment(id) {
    let mailrecipients = [];
    const response = await this.auditModel.findById(id);
  ...
  async getNcDataByAuditId(user, id) {
    // try {
    const activeUser = await this.prismaService.user.findFirst({
      where: { kcId: user.id },
    });
    ...
    // console.log('response', response);
    return { nc: response, count: response.length };
    // } catch (err) {}
  }
