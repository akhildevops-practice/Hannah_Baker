...
import { TicketSupportModule } from 'src/ticket-support/ticket-support.module';
...
export class OrganizationService {
  ...
  async createOrg(body: CreateOrgDto, file, res: any, token: any, req) {
    ...
    let logoUrl: string;
    if (file) {
      logoUrl = `${process.env.SERVER_IP}/${body.realm.toLowerCase()}/logo/${
        file.filename
      }`;
    }
    let createdOrg;
      ...
      if (createKcRealm.status === 201) {
        ...
        try {
          ...
          if (getKcRealmId.status === 200) {
            ...
            const { realm, instanceUrl, principalGeography, domain } = body;
  ...
  async getRequester(requesterId: number, url, password) {
    try {
      const response = await axios.get(`${url}/contacts/${requesterId}`, {
        auth: {
          username: password, // e.g., 'your_api_key_here'
          password: 'X', // Freshdesk requires 'X' as password
        },
        headers: {
      ...
      // console.error(`Failed to get requester ${requesterId}`);
      return null;
    }
  }

  // Helper: get company
  async getCompany(companyId: number, url, password) {
    try {
      const response = await axios.get(`${url}/companies/${companyId}`, {
        auth: {
          username: password, // API key from DB
          password: 'X', // Freshdesk requires 'X' as password
        },
        headers: {
          'Content-Type': 'application/json',
  ...
  // Helper: get agent
  async getAgent(agentId: number, url, password) {
    try {
      const response = await axios.get(`${url}/agents/${agentId}`, {
        auth: {
          username: password, // Your Freshdesk API key from DB
          password: 'X', // Always 'X' as per Freshdesk basic auth
        },
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // console.error(`Failed to get agent ${agentId}`);
      return null;
    }
  }

  async getFreshWorksData(
    {
      // orgName = 'HIIMS',
      ticketId,
      description = '',
    ...
    try {
      ...
      const freshdesk = await this.prisma.connectedApps.findFirst({
        where: { sourceName: 'freshdesk' },
      });
      // Step 1: Search company by orgName
      let orgName = activeUser?.organization.realmName.toLowerCase();
      let organizationId = '';
      ...
      const auth = {
        username: decodedPassword, // fetched from DB
        password: 'X', // usually 'X' for Freshdesk API
      };
      ...
      if (orgName) {
        const response = await axios.get(`${freshdesk?.baseURL}/companies`, {
          auth: {
            username: decodedPassword, // from DB
            password: 'X', // required by Freshdesk
          },
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        const matched = companies.find(
          (comp) => comp.name.toLowerCase() === orgName.toLowerCase(),
        );
        organizationId = matched?.id || '';
        // console.log('test',matched)
      }
      // Step 2: Build ticket search query
      let tickets: any = [];
      if (!ticketId) {
        const response = await axios.get(`${freshdesk?.baseURL}/tickets`, {
          auth,
          headers,
          params: {
        ...
        const response = await axios.get(
          `${freshdesk?.baseURL}/tickets/${ticketId}`,
          {
            auth,
            headers,
          },
      ...
      const enrichedTickets = await Promise.all(
        tickets.map(async (ticket) => {
          ...
          const agent = ticket.responder_id
            ? await this.getAgent(
                ticket.responder_id,
                freshdesk?.baseURL,
                decodedPassword,
          ...
          return {
            ...
            responder_name: agent?.contact?.name || '',
            company_name: companyName,
            category: ticket.custom_fields?.category || '',
            moduleName: ticket?.custom_fields?.cf_module || '',
            locationName: ticket?.custom_fields?.cf_location || '',
          };
        }),
      );
      ...
      // console.error('❌ Error fetching Freshdesk data:', err);
      return [];
    }
  }
  ...
  async getTicketMessages(ticketId: number) {
    try {
      // Step 1: Get user org + Freshdesk creds
      ...
      const baseUrl = freshdesk?.baseURL;
      ...
      const { data: conversations } = await axios.get(
        `${baseUrl}/tickets/${ticketId}/conversations`,
        { auth, headers },
      );

      // Step 3: Resolve user names/types
      const messages = await Promise.all(
        conversations.map(async (conv) => {
          ...
          try {
            if (conv.private || !conv.incoming) {
              const { data: agent } = await axios.get(
                `${baseUrl}/agents/${conv.user_id}`,
                { auth, headers },
              );
              user_name = agent?.contact?.name || 'Support Agent';
              user_type = 'agent';
            } else {
              const { data: contact } = await axios.get(
                `${baseUrl}/contacts/${conv.user_id}`,
                { auth, headers },
              );
            ...
            // silently fail and fallback to default name/type
          }
