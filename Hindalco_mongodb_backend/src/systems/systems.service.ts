...
export class SystemsService {
  ...
  async create(userKcId: string, createSystemDto: CreateSystemDto) {
    // try {
    const user = await this.userService.getUserInfo(userKcId);
    //console.log('user in ystem', user);
  ...
  async findAll(userKcId: any, skip: number, limit: number) {
    try {
      const response = [];
      const user = await this.userService.getUserInfo(userKcId.id);
  ...
  async AddNewClause(systemId: string, data: any) {
    try {
      ...
      // );
      // return system;
      const clauseadded = await this.ClauseModel.create(data);
      return clauseadded;
    } catch (err) {
      console.error(err);
      throw new InternalServerErrorException();
  ...
  async updateClauses(clauseId: string, data: any) {
    try {
      const system: any = await this.ClauseModel.findByIdAndUpdate(
        clauseId,
        data,
