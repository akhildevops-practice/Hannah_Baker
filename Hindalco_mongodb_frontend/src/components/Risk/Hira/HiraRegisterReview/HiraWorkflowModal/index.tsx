...
const HiraWorkflowModal = ({
  ...
  useEffect(() => {
    ...
    // if (!!reviewModal?.open && !!reviewModal?.isOnTour) {
    //   setTourOpenForWorkflow(true);
    // }
    console.log("checkrisk2 workflow page, modal", location?.state);
    console.log("checkrisknew in workflow modal appliedFilters", appliedFilters);
  ...
  const handleSubmitFormOld = async (values: any) => {
    try {
      ...
      if (hiraStatus === "open") {
        ...
        if (process.env.REACT_APP_REDIRECT_URL?.includes("adityabirla.com")) {
          url = `${process.env.REACT_APP_REDIRECT_URL}/risk/riskregister/HIRA/workflow`;
          hiraPageUrl = `${process.env.REACT_APP_REDIRECT_URL}/risk/riskregister/HIRA/${encodedJobTitle}/${reviewModal?.data?.entityId}`;
        } else {
          url = `${realmName}.${process.env.REACT_APP_REDIRECT_URL}/risk/riskregister/HIRA/workflow`;
          hiraPageUrl = `${realmName}.${process.env.REACT_APP_REDIRECT_URL}/risk/riskregister/HIRA/${encodedJobTitle}/${reviewModal?.data?.entityId}`;
        }
        ...
        const data = {
          ...
          jobTitle: reviewModal?.hiraInWorkflow?.jobTitle || selectedJobTitle,
        ...
        const res = await axios.post(
          `/api/riskregister/hira-register/createConsolidateEntry/${encodedJobTitle}/${userDetails?.organizationId}`,
          data
        );
        console.log("res", res);
        if (res.status === 200 || res.status === 201) {
          ...
          navigate(`/risk/riskregister/HIRA/${encodedJobTitle}`, {
            state : {
        ...
        let url,hiraPageUrl;
        const encodedJobTitle = encodeURIComponent(reviewModal?.hiraInWorkflow?.jobTitle || selectedJobTitle);
        if (process.env.REACT_APP_REDIRECT_URL?.includes("adityabirla.com")) {
          url = `${process.env.REACT_APP_REDIRECT_URL}/risk/riskregister/HIRA/workflow`;
          hiraPageUrl = `${process.env.REACT_APP_REDIRECT_URL}/risk/riskregister/HIRA/${encodedJobTitle}/${reviewModal?.data?.entityId}`;
        } else {
        ...
        const res = await axios.patch(
          `/api/riskregister/hira-register/updateConsolidatedEntry/${reviewModal?.hiraInWorkflow?._id}`,
          data
        );
        console.log("res", res);
        if (res.status === 200 || res.status === 201) {
        ...
        const data = {
          ...
          status: "REJECTED",
          organizationId: userDetails?.organizationId,
          createdBy : reviewModal?.hiraInWorkflow?.createdBy,
          jobTitle: reviewModal?.hiraInWorkflow?.jobTitle || selectedJobTitle,
          reviewers: reviewModal?.hiraInWorkflow?.reviewers || [],
          approvers: reviewModal?.hiraInWorkflow?.approvers || [],
        ...
        const res = await axios.patch(
          `/api/riskregister/hira-register/updateHiraConsolidatedForRejectedHira/${reviewModal?.hiraInWorkflow?._id}`,
          data
        );
        console.log("res", res);
        if (res.status === 200 || res.status === 201) {
  ...
  const handleSubmitForm = async (values: any) => {
    try {
      ...
      if (reviewModal?.status === "DRAFT") {
        ...
        console.log("checkrisk6 data in handlesubmitform while statys is DRAFT", data);
        ...
        console.log("checkrisk data status == IN REVIEW  in handlesubmitform", data);
        ...
        if (res.status === 200 || res.status === 201) {
          enqueueSnackbar(
            //   `Hira With Job Title ${reviewModal?.data?.jobTitle} Successfully Sent for Review`,
            res?.data?.message ||
              `Hira With Job Title ${reviewModal?.data?.jobTitle} Successfully Sent for Approval`,
            {
              variant: "success",
              autoHideDuration: 2500,
            }
          ...
          console.log("checkrisk data in handlesubmitform when status is IN_APPROVAL", data);
          const res = await axios.put(
            `/api/riskregister/hira/updateWorkflowStatus/${reviewModal?.data?.hiraId}`,
            data
          );
          ...
          if (res.status === 200 || res.status === 201) {
            enqueueSnackbar(
              res?.data?.message ||
                `Hira With Job Title ${reviewModal?.data?.jobTitle} Successfully Approved`,
              {
                variant: "success",
                autoHideDuration: 2500,
              }
          ...
          console.log("checkrisk4 data in handlesubmitform when status is REJECTED", data);
          const res = await axios.put(
            `/api/riskregister/hira/updateWorkflowStatus/${reviewModal?.data?.hiraId}`,
            data
          );
          if (res.status === 200 || res.status === 201) {
            enqueueSnackbar(res?.data?.message ||
                `Hira With Job Title ${reviewModal?.data?.jobTitle} Successfully Sent Back For Edit`,
              {
                variant: "success",
                autoHideDuration: 2500,
              }
        ...
        console.log("checkrisk4 data in handlesubmitform in STARTREVIEW", data);

        console.log(data, reviewModal?.data?.hiraId);
        const res = await axios.put(
