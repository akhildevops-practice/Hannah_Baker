...
import { Divider, Form, Modal, Upload, UploadProps } from "antd";
...
const HiraAreaMasterTab = () => {
  const [areas, setAreas] = useState<any>([]);
  const [areasCount, setAreasCount] = useState<any>(0);
  ...
  const [selectedAreaIdForDelete, setSelectedAreaIdForDelete] =
    useState<any>(null);
  ...
  // useEffect(() => {
  //   console.log("checkrisk areas", areas);
  // }, [areas]);
  ...
  const getAllImpactTypesByLocationId = async (
    ...
    try {
      const response = await axios.get(
        `api/riskconfig/getAllAreaMaster?locationId=${selectedLocation?.id}&orgId=${userDetails?.organizationId}&master=true&pagination=true&page=${page}&pageSize=${pageSize}`
      );
      // console.log(
      ...
      if (response?.data && !!response?.data?.data?.length) {
        ...
        setAreasCount(response.data?.count);
      } else {
  ...
  const handleOpen = (val: any) => {
    setOpen(true);
    setSelectedAreaIdForDelete(val);
  };
  ...
  const toggleEditState = (hazardId: any) => {
    const updatedHazards = areas.map((item: any) => {
      if (item.id === hazardId) {
  ...
  const handleAddImpact = async () => {
    ...
    setAreas([newHazard, ...areas]);
    setAreasCount((prevValue: any) => prevValue + 1);
  };

  const isImpactNameUnique = (name: any) => {
    return !areas.some((type: any) => {
      // console.log(
      //   "uniqueName",
      //   areas,
      //   name,
  ...
  const handlePutImpact = async (hazardId: any, item: any) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `api/riskconfig/updateAreaMaster/${hazardId}`,
        {
          name: item.name,
  ...
  const handleDeleteHazard = async () => {
    if (selectedAreaIdForDelete?.isNewEntry) {
      const updatedHazardTypes = areas.filter(
        (type: any) => type?.id !== selectedAreaIdForDelete?.id
      );
      setAreas(updatedHazardTypes);
      setAreasCount((prevValue: any) => prevValue - 1);
      handleClose();
    } else {
      try {
        const response = await axios.delete(
          `/api/riskconfig/deleteAreaMaster/${selectedAreaIdForDelete?.id}`
        );
        if (response.status === 200 || response.status === 201) {
          const updatedHazardTypes = areas.filter(
            (type: any) => type.id !== selectedAreaIdForDelete?.id
  ...
  const handleHazardNameChange = (event: any, hazardId: string) => {
    ...
    const updatedHazardTypes = areas.map((type: any) => {
      if (type.id === hazardId) {
        return {
          ...type,
  ...
  const handlePostImpact = async (item: any) => {
    ...
    try {
      ...
      const response = await axios.post(
        `/api/riskconfig/createAreaMaster`,
        hazard
      );
      if (response.status === 200 || response.status === 201) {
        const returnedId = response.data._id;
        // Use the returnedId as needed
        const updatedHazards = areas.map((type: any) => {
  ...
  const handleDescriptionChange = (event: any, hazardId: string) => {
    ...
    const updatedImpactTypes = areas.map((type: any) => {
      if (type.id === hazardId) {
        return {
          ...type,
...
export default HiraAreaMasterTab;
