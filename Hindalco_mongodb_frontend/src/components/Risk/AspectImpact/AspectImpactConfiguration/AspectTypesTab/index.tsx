...
const AspectTypesTab = () => {
  ...
  const getAllHazardTypesByLocationId = async (
    ...
    try {
      const response = await axios.get(
        `/api/aspect-impact/getAspectTypes?locationId=${selectedLocation?.id}&master=true&type=hazard&orgId=${userDetails?.organizationId}&pagination=true&page=${page}&pageSize=${pageSize}`
      );
      // console.log(
      ...
      enqueueSnackbar("Error in Aspect Types", {
        variant: "error",
      });
      setHazardTypes([]);
      setHazardTypeCount(0);
  ...
  const handlePutHazard = async (hazardId: any, item: any) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `/api/aspect-impact/updateAspectType/${hazardId}`,
        {
          name: item.name,
    ...
    if (selectedHazardTypeIdForDelete.isNewEntry) {
      ...
      try {
        const response = await axios.delete(
          `/api/aspect-impact/deleteAspectType/${selectedHazardTypeIdForDelete?.id}`
        );
        if (response.status === 200 || response.status === 201) {
          const updatedHazardTypes = hazardTypes.filter(
            (type: any) => type.id !== selectedHazardTypeIdForDelete?.id
          ...
          enqueueSnackbar("Error in Deleting Aspect Type", {
            variant: "error",
          });
          handleClose();
          return;
  ...
  const handlePostHazard = async (item: any) => {
    ...
    if (!isUnique) {
      enqueueSnackbar("Aspect Type with Same Name Already Exists!", {
        variant: "error",
      });
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        `/api/aspect-impact/createAspectType`,
        hazard
      );
      if (response.status === 200 || response.status === 201) {
          ...
          {!!hazardTypeCount ? (
                      ...
                      <TableCell style={{ width: "40%" }}>
                        {item.isEdit ? (
                          <TextareaAutosize
                            ...
                            value={item.name}
                            placeholder="Enter Aspect Type Name"
                            onChange={(e: any) =>
            ...
            <>
              ...
              <Typography align="center" className={classes.emptyDataText}>
                Let’s begin by adding a Aspect Type
              </Typography>
            </>
          )}
        </div>
