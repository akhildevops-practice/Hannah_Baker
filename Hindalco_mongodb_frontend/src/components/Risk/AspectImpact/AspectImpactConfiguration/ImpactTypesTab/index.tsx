...
const ImpactTypesTab = () => {
  ...
  const getAllImpactTypesByLocationId = async (
    ...
    try {
      const response = await axios.get(
        `/api/aspect-impact/getImpactTypes?locationId=${selectedLocation?.id}&master=true&type=impact&orgId=${userDetails?.organizationId}&pagination=true&page=${page}&pageSize=${pageSize}`
      );
      // console.log(
  ...
  const handlePutImpact = async (hazardId: any, item: any) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `/api/aspect-impact/updateImpactType/${hazardId}`,
        {
          name: item.name,
    ...
    if (selectedImpactedTypeIdForDelete?.isNewEntry) {
      ...
      try {
        const response = await axios.delete(
          `/api/aspect-impact/deleteImpactType/${selectedImpactedTypeIdForDelete}`
        );
        if (response.status === 200 || response.status === 201) {
          const updatedHazardTypes = impactTypes.filter(
            (type: any) => type.id !== selectedImpactedTypeIdForDelete
          );
  ...
  const handlePostImpact = async (item: any) => {
    ...
    // console.log("checkrisk hazardName", item);
    ...
    try {
      ...
      const response = await axios.post(
        `/api/aspect-impact/createImpactType`,
        hazard
      );
      if (response.status === 200 || response.status === 201) {
