...
const ActMasterTab = () => {
  ...
  const getAllActs = async (page: number = 1, pageSize: number = 10) => {
    try {
      const response = await axios.get(
        `/api/aspect-impact/getActs?&orgId=${userDetails?.organizationId}&pagination=true&page=${page}&pageSize=${pageSize}`
      );
      ...
      enqueueSnackbar("Error in getting Acts", {
        variant: "error",
      });
      setHazardTypes([]);
      setHazardTypeCount(0);
  ...
  const handleChangePage = (pageNumber: any, pageSize: any = rowsPerPage) => {
    ...
    getAllActs(pageNumber, pageSize);
  };
  ...
  const handlePutHazard = async (hazardId: any, item: any) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `/api/aspect-impact/updateAct/${hazardId}`,
        {
          name: item.name,
    ...
    if (selectedHazardTypeIdForDelete.isNewEntry) {
      ...
      try {
        const response = await axios.delete(
          `/api/aspect-impact/deleteAct/${selectedHazardTypeIdForDelete?.id}`
        );
        if (response.status === 200 || response.status === 201) {
          const updatedHazardTypes = hazardTypes.filter(
            (type: any) => type.id !== selectedHazardTypeIdForDelete?.id
          ...
          const sortedHazardTypes = updatedHazardTypes
            .slice()
            .sort((a: any, b: any) => a.name.localeCompare(b.name));
          setHazardTypes(sortedHazardTypes);
          setHazardTypeCount((prevValue: any) => prevValue - 1);
          getAllActs();
          handleClose();
        } else {
          enqueueSnackbar("Error in Deleting act", {
            variant: "error",
          });
          handleClose();
          return;
  ...
  const handlePostHazard = async (item: any) => {
    ...
    if (!isUnique) {
      enqueueSnackbar("Act with Same Name Already Exists!", {
        variant: "error",
      });
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(`/api/aspect-impact/createAct`, hazard);
      if (response.status === 200 || response.status === 201) {
      ...
      <div className={classes.root}>
        <Grid container spacing={2} alignItems="center" justify="flex-end">
          ...
          <Grid item style={{ marginBottom: "10px" }}>
            {isMCOE ? (
              <Tooltip title="Add New Corresponding Act">
                <Button
          ...
          {!!hazardTypeCount ? (
                  ...
                  {hazardTypes.map((item: any) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell style={{ width: "20%" }}>
                        <Typography>{item?.createdBy?.firstname + " " + item?.createdBy?.lastname}</Typography>
                      </TableCell>
                      <TableCell style={{ width: "20%" }}>
                      <Typography>{moment(item?.createdAt).format("DD/MM/YYYY HH:mm")}</Typography>
                      </TableCell>
            ...
            <>
              ...
              <Typography align="center" className={classes.emptyDataText}>
                Let’s begin by adding a Corresponding Act
              </Typography>
            </>
          )}
        </div>
