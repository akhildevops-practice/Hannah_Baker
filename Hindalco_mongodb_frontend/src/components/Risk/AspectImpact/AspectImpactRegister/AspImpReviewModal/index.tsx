  ...
  const handleSubmitForm = async (values: any) => {
    try {
      ...
      const res = await axios.post(
        `/api/aspect-impact/createReviewHistoryEntry`,
        data
      );
      // console.log("res", res);
      if (res.status === 200 || res.status === 201) {
        enqueueSnackbar(
          `Aspect Impact For Department ${userDetails?.entity?.entityName} Successfully Reviewed!`,
          {
            variant: "success",
      ...
      reloadListAfterSubmit(true);
      handleCloseModal();
    } catch (error: any) {
      console.log(error);
      enqueueSnackbar(error.message || "Failed to Review Aspect Impact", {
        variant: "error",
        autoHideDuration: 2500,
      });
    }
...
export default AspImpReviewModal;
