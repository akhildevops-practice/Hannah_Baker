...
function groupByLocation(data: any) {
  ...
  data.forEach((item: any) => {
    ...
    const entityData = {
      ...
      sectionWiseData: item.sectionWiseData, // Add the sectionWiseData here
    };
...
const mergeSectionDataWithEntities = (
  mainTableResult: any,
  sectionResults: any
) => {
  // Iterate through the mainTableResult and add corresponding section data
  return mainTableResult.map((entity: any) => {
    // Find all sections that match the current entity
    const matchingSections = sectionResults.filter(
      (section: any) => section.entityName === entity.entityName
    );

    // Add the matched sections as a new property
    return {
      ...entity,
      sectionWiseData: matchingSections,
    };
  });
};
...
const HiraDashboard = () => {
  ...
  const [hiraByEntityData, setHiraByEntityData] = useState<any>([]);
  ...
  const [riskTableDataLoading, setRiskTableDataLoading] = useState<any>(false);
  ...
  useEffect(() => {
    ...
    getHiraCountByEntity(false, true);
  ...
  useEffect(() => {
    ...
    if (!!activeTab || activeTab === 0) {
      ...
      getHiraCountByEntity(false, false);
      fetchStatusCountsByEntity(isPrimaryFilter, 1, 10);
    }
  }, [activeTab]);

  useEffect(() => {
    // console.log("checkdashboard useEffect selectedBusiness", selectedBusiness);
  }, [selectedBusiness]);
      ...
      if (res?.status === 200) {
        if (!!res?.data?.list && !!res?.data?.list?.length) {
          ...
          setShowTableModal({
            ...
            entity: tableFilters.entity ? true : false,
          });
        } else {
          setAllHiraTableData([]);
    ...
    try {
      ...
      if (res?.status === 200) {
        if (!!res?.data && !!res?.data?.mainTableResult?.length) {
          const newSectionData = mergeSectionDataWithEntities(
            res.data?.mainTableResult,
            res.data?.sectionResults
          );

          const groupedData = groupByLocation(newSectionData);
  ...
  const getHiraCountByEntity = async (
    isPrimaryFilterParam: any = true,
    defaultFilter: any = false
  ) => {
    try {
      ...
      const res = await axios.get(
        `/api/riskregister/hira/getHiraCountByEntity/${userDetails?.organizationId}?${queryString}`
      );
      // console.log(
      //   "checkdashboardnew data in getHiraCountByCondition",
      //   res.data
      // );
      if (res?.status === 200) {
        setHiraByEntityData(res.data);
        setGraphLoaders({
          ...graphLoaders,
          hiraByEntityGraph: false,
        });
      } else {
        // console.log("Error in getHiraCount", res);
        setHiraByEntityData([]);
  ...
  const handleClickFetch = (values: any) => {
    ...
    if (
      ...
      fetchStatusCountsByEntity(isPrimaryFilter, 1, 10);
      getHiraCountByEntity(true, false);
      ...
      {isLoading ? (
        ...
        <>
          ...
          <HiraGraphSection
            ...
            hiraByEntityData={hiraByEntityData}
