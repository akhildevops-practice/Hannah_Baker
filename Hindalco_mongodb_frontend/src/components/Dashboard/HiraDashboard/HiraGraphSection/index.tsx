...
import HiraEntityBarChart from "./HiraEntityBarChart";
type Props = {
  formData: any;
  hazardGraphData?: any;
...
const HiraGraphSection = ({
  ...
  const [isModalOpenForEntity, setIsModalOpenForEntity] = useState(false);

  const showModalForEntity = () => {
    setIsModalOpenForEntity(true);
  };
  const handleOkForEntity = () => setIsModalOpenForEntity(false);
  const handleCancelForEntity = () => setIsModalOpenForEntity(false);
    ...
    const dataForExcel = hiraList.map((item: any) => ({
      ...
      Approvers:
        item.approversDetails
          ...
          .join(", ") || "N/A",
      "Pending With": item?.pendingWith || "N/A",
      "Approved On": item?.approvedDate || "N/A",
    }));
      ...
      >
        ...
        >
          ...
          {matches ? (
            ...
            >
              <ArrowsAltOutlined onClick={showModalForHazard} />
            </div>
          ) : null}
        </Paper>
        <Paper
        ...
        >
          ...
          {matches ? (
            ...
            >
              <ArrowsAltOutlined onClick={showModalForCondition} />
            </div>
          ) : null}
        </Paper>
        <Paper
        ...
        >
          ...
          {matches ? (
            ...
            >
              <ArrowsAltOutlined onClick={showModalForRiskTable} />
            </div>
          ) : null}
        </Paper>
        <Paper
        ...
        >
          {graphLoaders?.entityGraph ? (
            <Skeleton active />
          ) : (
            <HiraEntityBarChart
              entityData={hiraByEntityData}
              setTableFilters={setTableFilters}
              isModalView={false}
            />
          )}
          {matches && (
            ...
            >
              <ArrowsAltOutlined onClick={showModalForEntity} />
            </div>
          )}
           <Modal
            title=""
            open={showTableModal?.entity}
            width={"auto"}
            footer={null}
            onCancel={() =>
              setShowTableModal({
        ...
        <Modal
          open={isModalOpenForEntity}
          onOk={handleOkForEntity}
          onCancel={handleCancelForEntity}
          // width={900}
          footer={null}
          style={{ display: "flex", justifyContent: "center" }}
          closeIcon={
