...
const DocInfoTab = ({
    ...
    formData?.docTypes?.map((value1: any) => {
      ...
      if (
        ...
        if (value1.documentTypeName === formData.doctypeName) {
          for (let appSystems of value1.applicable_systems) {
  ...
  const CLIENT_ID =
    "704472601416-99u42vhbmu2esdfljt10648gvslt51gn.apps.googleusercontent.com";
  const API_KEY = "AIzaSyAAgF1QNsst2LWWe8CkfHnfN3dSbDfNjpM";
                  ...
                  <div
                    ...
                    onClick={async () => {
                      if (process.env.REACT_APP_IS_OBJECT_STORAGE === "true") {
                        ...
                        if (formData?.downloadAccess) {
                          window.open(formData.documentLink, "_blank");
                        }
                      }
                    }}
                  >
                    <AttachFileIcon style={{ color: "#003059" }} />
                    <span
                      style={{
                        marginLeft: "5px",
                        color: "#003059",
