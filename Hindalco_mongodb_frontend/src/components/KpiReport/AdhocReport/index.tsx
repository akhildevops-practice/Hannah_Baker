...
import { Carousel, CollapseProps, DatePicker } from "antd";
    ...
    await axios(`/api/kpi-report/getSelectedReportInstance/${id}`)
      .then(async (res: any) => {
        ...
        if (res.data.result.reportFrequency === "Daily") {
          ...
          let quarter = await getFiscalQuarter(date, res.data.result.yearFor);
    ...
    try {
      ...
      if (result && result.status === 200) {
        ...
        for (let i = 0; i < quarters.length; i++) {
          ...
          var from = new Date(d1[2], parseInt(d1[1]) - 1, d1[0]).getTime(); // Convert to timestamp
          var to = new Date(d2[2], parseInt(d2[1]) - 1, d2[0]).getTime(); // Convert to timestamp
          var check = new Date(c[2], parseInt(c[1]) - 1, c[0]).getTime(); // Convert to timestamp
