...
const CaraRegistration = ({
  ...
  const getAllEntities = async () => {
    try {
      ...
      let data = await getEntityByLocationId(selectedLocation?.id);
  ...
  const getAllUserForEntity = async (entity: any) => {
    try {
      let data = await axios.get(`/api/cara/getAllUsersOfEntity/${entity}`);
  ...
  const getAllLocations = async () => {
    try {
      const res = await axios.get(
        `/api/aspect-impact/getAllLocation/${loggedInUser?.organizationId}`
      );
      ...
      <Row gutter={[12, 12]}>
        <Col span={24}>
          ...
          >
            ...
            <Input
              // rows={1} // Increase the number of rows to adjust the height
