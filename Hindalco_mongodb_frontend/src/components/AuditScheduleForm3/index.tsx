...
import { getAuditCreationTemplateById } from "../../apis/auditApi";
...
import { auditFormData, auditScheduleFormType } from "../../recoil/atom";
...
Props) {
  ...
  const handleAddEntityToScheduleTable = async (restoreId: any) => {
    ...
    if (scheduleFormType === "adhoc-create") {
      let defaultEntityHead = [] as any;
      let updateAuditScheduleData: any = { ...auditScheduleData };
      ...
      if (!!restoreId) {
        ...
        if (
          ...
          let timeOfTheFirstEntity =
            auditScheduleData?.AuditScheduleEntitywise?.[0]?.time;
          // let newTime = moment(timeOfTheFirstEntity).add(30, "minutes");
          let formattedTime = timeOfTheFirstEntity
            ? moment(timeOfTheFirstEntity).format("YYYY-MM-DDTHH:mm")
            : null;
  ...
  const handleEditClickForPlan = async (entityId: any) => {
    ...
    try {
      let auditPlanEntityWiseId = await fetchAuditPlanEntityWiseId(entityId);
