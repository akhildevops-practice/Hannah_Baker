...
import {
  getEntitiesByUserLocation,
  getEntityByLocationId,
} from "apis/entityApi";
import { getAllClauses } from "apis/clauseApi";
import { getAllDocuments, getAllDocumentsByEntity } from "apis/documentsApi";
...
function AuditInfoStepperForm({
  ...
  const findEntityHead = (payload: any[]) => {
    let entityHead = payload?.filter((item: any) => {
  ...
  useEffect(() => {
    ...
    if (id) {
      getAuditById(id)
        .then((res: any) => {
          var date = moment(res?.respond?.date ?? new Date());
        ...
        .then((response: any) => {
          let parsedClause = response?.auditedClauses?.map((clause: any) => {
            return {
              item: clause,
            };
          });
          let parsedDocuments = response?.auditedDocuments?.map((doc: any) => {
            return {
              item: doc,
            };
          });
  ...
  useEffect(() => {
    ...
    var subDate = moment().subtract(1, "days");
    var subDateComponent = subDate.format("YYYY-MM-DD");
    var timeComponent = date.format("HH:mm");
        ...
        >
          ...
          >
            ...
            >
              ...
              <Grid item sm={12} md={10} className={classes.formTextPadding}>
                <strong className={classes.labelStart}>* </strong>
            ...
            <Grid item container sm={4} md={4} style={{ height: "300px" }}>
              <Grid item sm={12} md={10} className={classes.formTextPadding}>
                <strong className={classes.labelStart}>* </strong>
                <strong className={classes.label}>Audited Dept/Vertical</strong>
              </Grid>
              <Grid item sm={12} md={10} className={classes.formBox}>
                <Select
                  style={{
              ...
              <Grid item sm={12} md={10} className={classes.formTextPadding}>
                ...
                <strong className={classes.label}>Schedule Date & Time </strong>
              </Grid>
              <Grid item sm={12} md={10} className={classes.formBox}>
                {/* <TextField
                  fullWidth
                ...
                <DatePicker
                  ...
                  style={{
                    ...
                    backgroundColor: "white",
                    color: "black",
                    pointerEvents: "none", // disables interaction while keeping visual style
                  }}
                  inputReadOnly // prevents editing but keeps normal text color
                  value={
                    formData.scheduleDate ? dayjs(formData.scheduleDate) : null
                  }
                  onChange={(value) => {
                    setFormData({
                      ...formData,
                      scheduleDate: value?.toDate() || null,
                    });
                  }}
