...
function EntityForm({
  ...
  const getLocation = async () => {
    try {
      let res = await axios.get(`/api/user/locationData`);
      if (isMr && !isOrgAdmin) {
        const unitIds = [
          ...
          <Grid container>
            ...
            <Grid item sm={12} md={8} className={classes.formBox}>
              <AutoComplete
                ...
                setFormData={setFormData}
                getSuggestionList={getSuggestionListUser}
                defaultValue={formData?.additionalAuditee?.length ? formData?.additionalAuditee : []}
                type="RA"
              />
            </Grid>
          </Grid>
          ...
          <Grid container>
            ...
            <Grid item sm={12} md={8} className={classes.formBox}>
              <AutoComplete
                ...
                setFormData={setFormData}
                getSuggestionList={getSuggestionListUser}
                defaultValue={formData?.notification?.length ? formData?.notification : []}
                type="RA"
              />
            </Grid>
            <Grid item sm={12} md={4} className={classes.formTextPadding}>
      ...
      {/* {!disableFormFields && (
        <div className={classes.buttonSection}>
          {isLoading ? (
            ...
            <CustomButton text="Save" handleClick={handleSubmit} />
          ) : (
            !isCreated && (
