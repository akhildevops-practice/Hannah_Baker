...
const CipTab = ({
  ...
  const totalCount =
  moduleNames.find((module) => module.name === "CIP")?.count || 0;


  useEffect(() => {
    // console.log("checkrisk cipData in cipData tab-->", cipData);
    setSelectedFilters([userDetails.locationId]);
    setSelectedEntityFilters([userDetails?.entity?.id]);

    setCount(totalCount);
    setTableData(cipData);
  }, [cipData, searchValue]);
  ...
  const fetchTableData = async (page: number=1, pageSize: number=100, reset:any) => {
    try {
      ...
      const res = await axios.get(
        `api/globalsearch/getCipSearchResultWithFilter?${queryString}`
      );
      if (res?.status === 200) {
        setTableData(res?.data?.data);
        setCount(res?.data?.count);
        updateModuleCount("CIP", res?.data?.count || 0);
      }
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "locationDetails",
      render: (_: any, record: any) => record?.locationDetails?.name,
      filterIcon: () =>
        selectedFilters.length > 0 ? (
          <FilterFilled size={16} style={{ color: "black", fontSize : "16px !important" }} />
        ) : (
          <FilterOutlined size={16} style={{ fontSize : "16px !important"}} />
        ),
            ...
            <div>
              ...
              <label>
                <input
                  ...
                  checked={allSelected}
                  onChange={(e) => {
                    let updatedFilters = e.target.checked
  ...
  const rowSelection = {
    onChange: (selectedRowKeys: any, selectedRows: any) => {
      console.log("checkrisk inside row selection cipData tab", selectedRows);
