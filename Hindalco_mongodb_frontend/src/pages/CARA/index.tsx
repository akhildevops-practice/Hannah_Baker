...
${data?.correctedDate}
  ...
  const getData = async (url: string) => {
    try {
      ...
      if (data?.data) {
        let dataSrc = data?.data?.data?.map((el: any, index: number) => {
          return {
  ...
  const handleSearch = async (searchValue: string) => {
    ...
    try {
      ...
      let res = await axios.get("/api/cara/searchCapa", {
        params: newPayload,
      });
  ...
  return (
    <>
            ...
            >
              ...
              {item.actionItem.length > 0 ? (
                <Accordion className={classes.headingRoot}>
                  <AccordionSummary
                    expandIcon={<ExpandMoreIcon style={{ padding: "3px" }} />}
      ...
      {/* <div style={{ position: "fixed", top: "79px", right: "120px" }}>
        <Tooltip title="Help Tours!">
          <TouchAppIcon
            style={{ cursor: "pointer" }}
            onClick={() => {
              setOpenTourForCapa(true);
            }}
          />
        </Tooltip>
      </div> */}
