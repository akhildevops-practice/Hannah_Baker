...
function Dashboard({}: Props) {
  ...
  const getOrgName = async () => {
    try {
      let res = await axios.get(`api/organization/${realmName}`);
  ...
  useEffect(() => {
    ...
    if (!isOrgAdmin) {
      (async () => {
        try {
          let { data }: any = await axios.get(`api/user/getUserInfo`);
          setSearch({
            ...searchValues,
