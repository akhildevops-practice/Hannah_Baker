...
function UserMaster({ type = true }: Props) {
      ...
      if (res?.data?.data) {
        ...
        let values = res?.data?.data?.map((item: any) => {
          ...
          const mappedRoles = item?.assignedRole?.map((role: any) => {
            if (role.role === "MR") {
              return { ...role, role: "IMSC" };
            } else if (role.role === "ORG-ADMIN") {
              return { ...role, role: "MCOE" };
            }
            return role;
          });
  ...
  const handleDelete = async () => {
    ...
    setIsLoading(true);
    try {
      let res = await axios.delete(`api/user/${deleteLoc.id}/${orgName}`);
  ...
  const handleChangePage = (page: any, pageSize: any = rowsPerPage) => {
    ...
    let url = formatDashboardQuery(`/api/user/allusers/${orgName}`, {
      ...searchValue,
      location: selectedLocation,
      entity: selectedEntity,
      page: page,
      ...
      >
        {isMr && matches && (
          <Tooltip title="Delegate">
            ...
            >
              <FindReplaceOutlinedIcon style={{ marginRight: "8px" }} />
              Delegate
            </Button>
          </Tooltip>
        )}
        ...
        {isMr && matches && selectedDeptUsers?.length > 0 && (
          <Tooltip title="Assign Department">
            ...
            >
              ...
              >
                <AssignmentLateOutlinedIcon style={{ marginRight: "8px" }} />
                Assign
              </Button>
