...
function LocationCorp({}: Props) {
  ...
  const columns: ColumnsType<any> = [
    ...
    },
    {
      ...
      filterDropdown: ({
        ...
        console.log("uniqueFunctionName", uniqueFunctionNamesArray, data);
        return (
          <div style={{ padding: 8 }}>
  ...
  useEffect(() => {
    let encodedFunctions = encodeURIComponent(JSON.stringify(filteredValues));
    const url = formatQuery(
  ...
  const handleApply = () => {
    ...
    const url = formatQuery(
      `/api/location/${orgName}/Function`,
      { ...searchValue, page: 1, limit: rowsPerPage, type },
      ["query", "page", "limit"]
    );
    getData(url);
  };
  ...
  const handleChangePage = (page: any) => {
    ...
    const url = formatQuery(
      `/api/location/${orgName}/Function`,
      { ...searchValue, page: page, limit: rowsPerPage, type },
