...
import { hiraConfig } from "schemas/riskConfigSchema";
...
const HiraRegisterReviewPage = () => {
  ...
  const stepsForWorkflowTour: TourProps["steps"] = [
    {
      title: "Start Review",
      description: "Please Click this button to Send HIRA For Review",
      target: () =>
        refStartReviewButton.current ? refStartReviewButton.current : null,
      nextButtonProps: {
        style: { display: "none" },
      },
      onPrev: () => {
  ...
  useEffect(() => {
    if (params && params?.hiraId && location?.state?.hiraLocationId && location?.state?.hiraEntityId) {
      ...
      console.log("checkrisk6 hiraid in useeffect review page", params);
      ...
      const fetchAllData = async () => {
        try {
          ...
          const [existingRiskConfigParam, areaOptionsParam, sectionOptionsParam, locationWithUsersParam] = await Promise.all([
            fetchHiraConfig(),
      ...
      if(params?.hiraId){
        setIsPageLoading(true);
        console.log("checkrisk6 hiraid in useeffect just hiraId review page", params);
        
        const fetchAllData = async () => {
          try {
            const [existingRiskConfigParam, locationWithUsersParam] = await Promise.all([
              fetchHiraConfig(),
              fetchUsersByLocation()
            ]);
            // Call all API functions in parallel
  ...
  const checkIfLoggedInUserCanReview = () => {
    console.log("checkrisk4 hiraheader form data in checkIdLoggedUserCanReview", hiraHeaderFormData);
    const ongoingWorkflowDetails = hiraHeaderFormData?.workflow?.find((workflow:any)=> workflow?.cycleNumber === hiraHeaderFormData?.currentVersion+1)
    console.log("checkrisk4 ongoingWorkflowDetails checkIdLoggedUserCanReview", ongoingWorkflowDetails);
  ...
  const checkIfLoggedInUserCanApprove = () => {
    console.log("checkrisk4 hiraheader form data checkIdLoggedUserCanApprove", hiraHeaderFormData);
    const ongoingWorkflowDetails = hiraHeaderFormData?.workflow?.find((workflow:any)=> workflow?.cycleNumber === hiraHeaderFormData?.currentVersion+1)
    console.log("checkrisk4 ongoingWorkflowDetails checkIdLoggedUserCanApprove", ongoingWorkflowDetails);
  ...
  const getReviewedByDetails = () => {
    ...
    if(latestOngoingWorkflow) {
      return {
        ...
        reviewers : latestOngoingWorkflow?.reviewersDetails?.map((reviewer:any)=> reviewer?.firstname + " " + reviewer?.lastname).join(", ")
      }
  ...
  const getApprovedByDetails = () => {
    ...
    if(latestOngoingWorkflow) {
      return {
        ...
        approvers : latestOngoingWorkflow?.approversDetails?.map((approver:any)=> approver?.firstname + " " + approver?.lastname).join(", ")
      }
            ...
            >
              <div style={{ display: "flex", gap: "15px" }}>
                {hiraHeaderFormData?._id &&
                 ( hiraHeaderFormData?.workflowStatus === "DRAFT" || hiraHeaderFormData?.workflowStatus === "REJECTED" || hiraHeaderFormData?.workflowStatus === "APPROVED") && (
                    <Button
                      style={{
                        display: "flex",
                        alignItems: "center",
                ...
                {showSendBackForEditButton() && (<Button
                      style={{
                        display: "flex",
                        alignItems: "center",
                        backgroundColor: "#003566",
                ...
                >
                  ...
                  >
                    <b>Reviewers :</b>{" "}
                    {getReviewedByDetails()?.reviewers}                  
                    <br />
                    <b>Approvers :</b>{" "}
                    {getApprovedByDetails()?.approvers}
                  </div>
                </Popover>
              ...
              {isHeaderDataLoading ? (
                ...
                >
                  ...
                  <Descriptions.Item label="Area : ">
                    <Typography>
                      {hiraHeaderFormData?.areaName || "N/A"}
                    </Typography>
                  </Descriptions.Item>
                  <Descriptions.Item label="Section : ">
                    <Typography>
                      {hiraHeaderFormData?.sectionName || "N/A"}
                    </Typography>
                  </Descriptions.Item>
                  <Descriptions.Item label="Routine/Non Routine:">
                    <Typography>
                      {hiraHeaderFormData?.riskTypeName || "N/A"}
                    </Typography>
                  </Descriptions.Item>
                  <Descriptions.Item label="Condition:">
                    <Typography>
                      {hiraHeaderFormData?.conditionName || "N/A"}
                    </Typography>
                  </Descriptions.Item>
