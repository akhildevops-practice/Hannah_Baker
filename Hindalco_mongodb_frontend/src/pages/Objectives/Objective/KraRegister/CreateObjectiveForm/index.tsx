...
const CreateObjectiveForm = ({
  ...
  let isMR = checkRoles("MR") && !!userDetail?.location?.id;
  let isMCOE = checkRoles("ORG-ADMIN") && !!userDetail?.location?.id;
  ...
  const fetchLocations = async () => {
    try {
      const res = await axios.get(
        `/api/aspect-impact/getAllLocation/${userDetail?.organizationId}`
      );
  ...
  const fetchEntities = async () => {
    try {
      ...
      let data = await getEntityByLocationId(selectedLocation);
