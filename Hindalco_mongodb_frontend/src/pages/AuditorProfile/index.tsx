...
import { Attachment, DeleteOutline } from "@material-ui/icons";
import {
  Box,
  Button,
  CircularProgress,
...
// import formatDashboardQuery from "utils/formatModernQuery";
import {
...
const AuditorProfile: React.FC = () => {
  ...
  const setParsedAuditType = (data: any) => {
    ...
    let parsedAuditType = data.auditType.map((type: any) => {
  ...
  const fetchAllProficiency = async () => {
    try {
      let res = await axios.get(`/api/audit-settings/getAllProficiency`);
      const val = res?.data.data.map((item: any) => {
        return {
