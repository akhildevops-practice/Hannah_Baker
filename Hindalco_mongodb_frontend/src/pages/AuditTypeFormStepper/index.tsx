...
import AuditTypeForm2 from "components/AuditTypeForm2";
import AuditTypeForm3 from "components/AuditTypeForm3";
...
const AuditTypeFormStepper = ({ data, values }: Props) => {
  ...
  const getFindingsData = () => {
    getAllAuditFindings(page, rowsPerPage).then((response: any) => {
      ...
      let DataByHead = FinalDataMapped.headings.map((item: any, index: any) => {
        return {
  ...
  const getScopes = async () => {
    await axios(`/api/auditPlan/getEntityByLocation`)
      .then((res) => {
        ...
        setScopes([...res.data, unit, corpFunc]);
      })
      .catch((err) => console.error(err));
  };
