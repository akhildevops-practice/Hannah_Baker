...
Props) => {
    ...
    try {
      ...
      if(aceoffixEdit){
        form = toFormData({
          ...
          aceoffixUrl : process.env.REACT_APP_API_URL + `/Temp/editTemp/${formData.documentLinkNew.split("/").pop()}`,
  ...
  const getDocData = async () => {
    try {
      ...
      const filteredOptions = buttonOptionsResponse?.data?.filter((option: string) => {
        return process.env.REACT_APP_IS_ACEOFFIX === "true" || option !== "Inline Edit";
      });
      const disableBtnFor = ["In Review", "In Approval"];
      const newItems = filteredOptions.map(
