...
import { InputAdornment, Switch, TextField, Tooltip } from "@material-ui/core";
import {
  getAllSuggestions,
  getAuditTemplate,
...
const NewAuditTemplate = () => {
  ...
  useEffect(() => {
    ...
    let accountInfo = parseToken(getToken());
  ...
  const handleSubmit = async (option: string) => {
    let { createdAt, updatedAt, _id, ...payload }: any = template;
  ...
  const handleSectionChange = (e: any, index: any) => {
    let sectionData = JSON.parse(JSON.stringify(template));
    setTemplate((prev: any) => {
  ...
  const handleQuestionChange = (e: any, index: any, questionIndex: any) => {
    let questionData = JSON.parse(JSON.stringify(template));
