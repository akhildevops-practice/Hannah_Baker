...
import CheckboxAutocomplete from "components/newComponents/inputs/CheckboxAutocomplete";
      ...
      <Grid container spacing={2}>
        <Grid item xs={12}>
          <Grid container alignItems="center" justifyContent="space-between">
            ...
            >
              {/* {isOrgAdmin && (
                <Tooltip title="Export Kpis">
                  <GetAppIcon
                    onClick={() => exportKpis(false)}
        ...
        <Grid item xs={12} md={8}>
          ...
          {/* {((isUserSelected && deptId !== "All") ||
            ...
            <Tooltip title="Import Historic Data">
              <Button
                ...
                style={{ marginLeft: "10px" }}
                color="primary"
                onClick={() => setImportHistoricModel({ open: true })}
              >
          ...
          {importHistoricModel.open && (
            <Modal
              title="Import Kpi Data"
              open={importHistoricModel.open}
              onCancel={() => setImportHistoricModel({ open: false })}
              onOk={() => {
                importHistoricKpis();
                setImportHistoricModel({ open: false });
              }}
            >
              <Form.Item name="file" label={"Attach File: "}>
                <Upload
              ...
              <Tooltip title="Click to download a sample template file">
                <a
                  href="#"
                  onClick={getHistoricImportTemplate}
                  style={{
          ...
          {/* {filters.source && tableData.length > 0 && (
            // &&
            // ((isMR && userDetail.location.id === unitId) ||
            //   isUserHead ||
            //   isUserSelected ||
            //   isMCOE)
            //commented for hindalco as here, kpi creators will be automatically the report authors
            <Tooltip title="Assign Report Creators">
              <Button
                variant="contained"
                style={{ marginLeft: "10px" }}
                color="primary"
